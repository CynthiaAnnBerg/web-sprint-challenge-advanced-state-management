...
Context provides a way to share values like these between components without having to explicitly pass a prop through every level of a tree. Context API allows us to create whats known as a context object. 

2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
Actions- packets of information that contain an action type and associated data. 
Reducers- an alternative to useState, a reducer is a function that determines changes to an application's state. It uses the action it receives to determine this change
Store- everything that changes within your app is represented by a single JavaScript Object known as the store. 
The store is known as a 'single source of truth' because it contains our state for our application. 

3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?
Redux-thunk seperates the node package, makes flow asynchronous and makes API calls for action creators. 

4. What is your favorite state management system you've learned and this sprint? Please explain why!
My favorite state management system I've learned is Redux because it provides a single source of truth for our data within our application. 
